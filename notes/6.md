...
Aside: note that it is more common to implement assignments by placing all variables on the stack (using `alloca`) and modifying the values directly with `store` instructions. This is an approach that practically relies on an LLVM optimization pass to place values from the stack in registers, so it could be considered inferior. In practice, this pass works very well and it reduces IR generation complexity quite a bit, so it is probably better.
