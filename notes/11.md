...
The solution is to introduce a custom loop epilog block after the loop body. The loop body will uncoditionally jump to the epilog, which will conditionally restart the loop.
...
Supporting a real program is a big achievement, so let's celebrate by adding another test case—problem 2.07, which requires us to use Euclid's algorithm to determine the GCD of two numbers.
